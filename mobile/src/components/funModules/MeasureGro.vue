                ...
                <div class="table-wrap">
                    <table>
                        <tbody>
                            ...
                            <template v-else>
                                <tr v-for="(item,index) in measureGro.measureGroInfoList" :key="index" v-bind:class='{measureGroLibg:index==measureGro.isactive}' @click="showMeasureGro(item.measureInfo,index)">
                                    <td>{{item.name}}</td>
                                </tr>
                            </template>
                        </tbody>
    ...
    export default {
        ...
        methods: {
            showMeasureGro: function(measureInfo, index) {
                ...
                let dimensionBusiness = new window.Web3D.DimensionBusiness();
                let cancel = new window.Web3D.CancelBuiness();
                cancel.getCancel(window.viewer);
                if (measureInfo.type == 'point') dimensionBusiness.drawLines(window.viewer, measureInfo);
                else if (measureInfo.type == 'face') dimensionBusiness.drawLines(window.viewer, measureInfo);
                else if (measureInfo.type == 'line') dimensionBusiness.drawLines(window.viewer, measureInfo);
            },
        },
    };
</script>