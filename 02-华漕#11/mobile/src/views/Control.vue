    ...
    <div class="control">
        <div class="drawing-wrap">
            ...
            <v-row class="text-center mx-5 j-control-legend" justify="center">
                <div class="j-legend"><span class="bg-color-grey"></span>未检查</div>
                <div class="j-legend"><span class="bg-color-orange"></span>未整改</div>
                <div class="j-legend"><span class="bg-color-green"></span>已整改</div>
                <div class="j-legend"><span class="tipstar" style="margin-top:-20px">*</span>必查点</div>
            ...
            <div class="j-control">
                <v-card class="mx-auto ma-3 mlr4 j-control-card" max-width="374"
                    ...
                    <v-card-text>
                        <v-row>
                            <div class="j-control-node bg-color-grey" v-if="(item.commentNumTotal == 0)">
                                <span v-if="item.mustBeChecked" class="tipstar">*</span>
                                <span v-if="!item.mustBeChecked" class="tipstar"></span>
                                <v-btn class="j-control-node-btn bg-color-grey" depressed
                                    @click="handleGoResource(item.controlPointSort)">{{ item.controlPoint }}></v-btn>
                            </div>
                            <div class="j-control-node bg-color-green" v-if="item.commentNumTotal != 0 && item.commentNumUnconfirmed == 0">
                                <span v-if="item.mustBeChecked" class="tipstar">*</span>
                                <span v-if="!item.mustBeChecked" class="tipstar"></span>
                                <v-btn class="j-control-node-btn bg-color-green" depressed
                                    @click="handleGoResource(item.controlPointSort)">{{ item.controlPoint }}</v-btn>
                            </div>
                            <div class="j-control-node bg-color-orange" v-if="item.commentNumUnconfirmed != 0">
                                <span v-if="item.mustBeChecked" class="tipstar">*</span>
                                <span v-if="!item.mustBeChecked" class="tipstar"></span>
...
export default {
    ...
    mounted() {
        ...
        if (this.controlSort == 'deliver') document.title = API.docTitle + '-交付层检查';
